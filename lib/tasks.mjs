...
function computeWallClock(results, sessions, noopThresholdMs, rangeStartMs = 0, rangeEndMs = Infinity) {
  ...
  for (const win of merged) {
    ...
    for (const { entries } of sessions) {
      ...
      wallIdle += summary.total_idle_ms || 0;
      wallTyping += summary.total_typing_ms || 0;
      wallPrompts += summary.prompts || 0;
    }
  }

  const wallUser = wallIdle + wallTyping;
  ...
  return { wallAgent, wallIdle, wallTyping, wallUser, wallPrompts, wallTotal, overlapMs };
}
...
function renderTerminal(resultsByProject, meta) {
  ...
  for (const [proj, projData] of resultsByProject) {
    ...
    if (wallClock) {
      ...
      const wcIdle = formatMs(wallClock.wallIdle).padStart(8);
      const wcTyping = formatMs(wallClock.wallTyping).padStart(8);
      const wcTotal = formatMs(wallClock.wallTotal).padStart(9);
      const wcPrompts = String(wallClock.wallPrompts).padStart(6);
      console.log(`      ${padRight('Wall-clock (deduplicated)', 42)} ${wcAgent}  ${wcIdle}  ${wcTyping}  ${wcTotal}  ${wcPrompts}`);
      if (wallClock.overlapMs > 0) {
...
function renderMarkdown(resultsByProject, meta) {
  ...
  for (const [proj, projData] of resultsByProject) {
    ...
    if (wallClock) {
      lines.push('');
      lines.push(`**Wall-clock (deduplicated):** Agent ${formatMs(wallClock.wallAgent)} | Idle ${formatMs(wallClock.wallIdle)} | Typing ${formatMs(wallClock.wallTyping)} | Combined ${formatMs(wallClock.wallTotal)} | ${wallClock.wallPrompts} prompts`);
      if (wallClock.overlapMs > 0) {
