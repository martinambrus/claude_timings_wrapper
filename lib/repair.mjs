...
function repairEntries(entries, thresholdMs) {
  ...
  for (const entry of entries) {
    ...
    if (entry.event === 'background_agent_stop' && entry.agent_work_ms && entry.agent_work_ms > thresholdMs) {
      fixes.push(`background_agent_stop.agent_work_ms: ${formatMs(entry.agent_work_ms)} → 0`);
      // Overnight background agent "work" is unattributable — discard
      if (entry.idle_correction_ms) {
        entry.idle_correction_ms = 0;
      }
      entry.agent_work_ms = 0;
      changed = true;
    }
...
export function repairSessions(options = {}) {
  ...
  for (const file of files) {
    ...
    const { repaired, changed: eventChanged, fixes } = repairEntries(entries, thresholdMs);
    let needsRepair = eventChanged;

    // Also detect sessions where session_end totals have large idle/agent
    // that isn't in individual events (final partial phase anomaly)
    const endEntry = entries.find(e => e.event === 'session_end');
    if (!needsRepair && endEntry) {
      // Check if session_end has large values not explainable by events
      const eventIdle = entries.reduce((sum, e) => sum + (e.idle_ms || 0), 0);
      const eventAgent = entries.reduce((sum, e) => sum + ((e.event === 'agent_stop' || e.event === 'agent_interrupt' || e.event === 'agent_stall' || e.event === 'extra_usage_limit') ? (e.agent_work_ms || 0) : 0), 0);
      const endIdle = endEntry.total_idle_ms || 0;
      const endAgent = endEntry.total_agent_ms || 0;
      const idleDelta = Math.abs(endIdle - eventIdle);
      const agentDelta = Math.abs(endAgent - eventAgent);
      if (idleDelta > thresholdMs) {
        fixes.push(`session_end idle residual: ${formatMs(idleDelta)} (not in events, discarding)`);
        needsRepair = true;
      }
      if (agentDelta > thresholdMs) {
        fixes.push(`session_end agent residual: ${formatMs(agentDelta)} (not in events, discarding)`);
        needsRepair = true;
      }
    }
    ...
    const newEnd = recomputeSummary(repaired);
    newEnd.ts = repaired[repaired.length - 1].ts || new Date().toISOString();
    repaired.push(newEnd);
