 ...
 */
export function clipMs(periodStartMs, periodEndMs, rangeStartMs, rangeEndMs) {
 ...
 */
export function recomputeForDateRange(entries, rangeStartMs, rangeEndMs, noopThresholdMs) {
  let totalIdleMs = 0;
  let totalTypingMs = 0;
  let totalAgentMs = 0;
  let prompts = 0;
  ...
  for (const entry of entries) {
    ...
    if (entry.event === 'agent_stop' || entry.event === 'agent_interrupt') {
      if (entry.agent_work_ms) {
    ...
    if (entry.event === 'session_paused') {
      if (entry.idle_before_pause_ms) {
        const periodStart = eventMs - entry.idle_before_pause_ms;
        const clipped = clipMs(periodStart, eventMs, rangeStartMs, rangeEndMs);
        totalIdleMs += clipped;
      }
      currentPhase = 'idle';
    ...
    if (entry.event === 'session_resumed') {
      currentPhase = 'idle';
      phaseStartMs = eventMs;
    }
...
export function matchesProject(cwd, filter) {
  if (!filter) return true;
  const name = projectName(cwd);
  return name.toLowerCase() === filter.toLowerCase();
}

export function extractFlags(args) {
  let projectFilter = null;
  let noNoop = false;
  let noopThresholdMs = null;
  let unattributedPrompts = false;
  let attributeLlm = false;
  let attributeModel = null;
  let maxBudgetUsd = null;
  let attributeDebug = false;
  let attributeTimeoutMs = null;
  const remaining = [];
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--project' && i + 1 < args.length) {
      ...
      i++;
    } else if (args[i] === '--unattributed-prompts') {
      unattributedPrompts = true;
    } else if (args[i] === '--attribute-llm') {
      attributeLlm = true;
      unattributedPrompts = true;
    } else if (args[i] === '--attribute-model' && i + 1 < args.length) {
      attributeModel = args[i + 1];
      i++;
    } else if (args[i] === '--max-budget-usd' && i + 1 < args.length) {
      const n = parseFloat(args[i + 1]);
      if (!Number.isNaN(n) && n > 0) maxBudgetUsd = n;
      i++;
    } else if (args[i] === '--attribute-debug') {
      attributeDebug = true;
    } else if (args[i] === '--attribute-timeout' && i + 1 < args.length) {
      const n = parseInt(args[i + 1], 10);
      if (!Number.isNaN(n) && n > 0) attributeTimeoutMs = n * 1000;
      i++;
    } else {
  ...
  return {
    projectFilter, noNoop, noopThresholdMs, remaining,
