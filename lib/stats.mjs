...
function extractProjectFilter(args) {
  ...
  return { projectFilter, remaining };
}

export function showStats(args) {
  const { projectFilter, remaining } = extractProjectFilter(args);
  ...
  if (range.mode === 'current') {
    ...
    for (let i = files.length - 1; i >= 0; i--) {
      const summary = readSessionSummary(files[i]);
      if (summary && summary.prompts > 0) {
        console.log('=== Claude Code Timing Stats ===');
  ...
  for (const [cwd, projSummaries] of sorted) {
    ...
    console.log(`-- ${projectName(cwd)} (${projSummaries.length} sessions, ${prompts} prompts) --`);
    console.log(`   ${cwd}`);
