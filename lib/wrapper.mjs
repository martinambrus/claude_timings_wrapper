  ...
  process.stdin.on('data', (data) => {
    ...
    if (buf.length === 1 && buf[0] === 0x0d) {
      if ((state === STATE.INITIAL || state === STATE.USER_TYPING) && typingStartTime !== null) {
        ...
        // User submitted a steering prompt while agent is working.
        // Track the typing time but keep the agent timer running —
        // the agent won't finish until the Stop hook fires.
        const typingMs = Date.now() - typingStartTime;
        totalTypingMs += typingMs;
        ...
        typingStartTime = null;
        // Stay in AGENT_WORKING — phaseStart unchanged
      }
      return;
    }
